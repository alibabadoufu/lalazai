"""
Refinitiv Chat Data Preprocessor.
This module handles cleaning and standardization of raw Refinitiv chat data.
"""
...
class RefinitivChatPreprocessor:
    """
    Preprocessor for Refinitiv chat data.
    
    Handles data cleaning, standardization, and validation specific to Refinitiv format.
    """
    ...
    def process(self, raw_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Process raw Refinitiv chat data.
    ...
    def _process_single_record(self, record: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        """
        Process a single Refinitiv chat record.
        ...
        """
        # Clean and standardize the record (adapt to Refinitiv format)
        cleaned_record = {
            ...
            "channel": record.get("channel", "refinitiv_chat"),
            "room_name": self._clean_string(record.get("room_name")),
    ...
    def _parse_timestamp(self, timestamp: Any) -> Optional[datetime]:
        """Parse timestamp from various Refinitiv formats."""
        ...
        if isinstance(timestamp, str):
            # Refinitiv-specific timestamp formats
            formats = [
                "%Y-%m-%d %H:%M:%S",
                "%Y-%m-%d %H:%M",
                "%Y-%m-%dT%H:%M:%S",
                ...
                "%d/%m/%Y %H:%M:%S",
                "%m/%d/%Y %H:%M:%S",
                "%Y%m%d %H:%M:%S"  # Refinitiv sometimes uses this format
            ]
            ...
            self.logger.warning(f"Unable to parse Refinitiv timestamp: {timestamp}")
            return None
    ...
    def _clean_message_content(self, content: Any) -> str:
        ...
        # Handle Refinitiv-specific formatting (e.g., RIC codes)
        content = self._normalize_refinitiv_codes(content)
        
        return content

    def _normalize_refinitiv_codes(self, content: str) -> str:
        """Normalize Refinitiv-specific codes and symbols."""
        # Normalize Reuters Instrument Codes (RICs)
        # Example: "EUR=" becomes "EUR/USD"
        content = re.sub(r'(\w{3})=', r'\1/USD', content)
        
        # Normalize other common Refinitiv symbols
        content = content.replace('.L', ' (London)')
        content = content.replace('.N', ' (NYSE)')
